...
# [1,3]
    ...
    #print(row[0],min(Average_change))
...
#Input the results
    ...
    textfile.write(f'\nTotal: {(Profit_loss)}')
    textfile.write(f'\nAverage Change: $ {(Average_ans)}')
    textfile.write(f'\nGreatest Increase in Profits: {Month_tracker[index]} (${max(Average_change)})')
    textfile.write(f'\nGreatest Decrease in Profits: {Month_tracker[index2]} (${min(Average_change)})')
